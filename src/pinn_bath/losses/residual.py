...
def swe_residual(
    ...
    *,
    ...
    if spatial_dim == 1 and not has_t:
        return _residual_1d_steady(form, coords, fields, g)
    if spatial_dim == 1 and has_t:
        return _residual_1d_transient(form, coords, fields, g)
    if spatial_dim == 2 and has_t:
        return _residual_2d_transient(form, coords, fields, g)
...
def _residual_1d_transient(
    ...
    if form == "prim_cons":
        ...
        r_mom_prim = u_t + u * u_x + g * (h_x + zb_x)
        return {
            "cont": r_cont_prim,
            "mom_x": u * r_cont_prim + h * r_mom_prim,
        }
    if form == "conservative":
        ...
        return {
            ...
            "mom_x": _grad(hu, t) + _grad(flux, x) + g * h * zb_x,
        }
    raise ValueError(f"unknown residual form: {form!r}")
...
def _residual_2d_transient(
    ...
    if form == "primitive":
        ...
        return {
            ...
            "mom_x": u_t + u * u_x + v * u_y + g * (h_x + zb_x),
            "mom_y": v_t + u * v_x + v * v_y + g * (h_y + zb_y),
        }
    if form == "prim_cons":
        h_x, h_y, h_t = _grad(h, x), _grad(h, y), _grad(h, t)
        u_x, u_y, u_t = _grad(u, x), _grad(u, y), _grad(u, t)
        ...
        r_mom_x_prim = u_t + u * u_x + v * u_y + g * (h_x + zb_x)
        r_mom_y_prim = v_t + u * v_x + v * v_y + g * (h_y + zb_y)
    ...
    if form == "conservative":
        ...
        return {
            ...
            "mom_x": _grad(hu, t) + _grad(flux_xx, x) + _grad(flux_xy, y) + g * h * zb_x,
            "mom_y": _grad(hv, t) + _grad(flux_xy, x) + _grad(flux_yy, y) + g * h * zb_y,
        }
    raise ValueError(f"unknown residual form: {form!r}")
